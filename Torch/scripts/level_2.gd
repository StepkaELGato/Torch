...
@onready var door_level_3: Area2D = $Door_to_Level3/doorLevel3  # Получаем ссылку на Area2D двери для перехода на уровень 3
...
func _ready() -> void:
	...
	if KillStates:
		KillStates.set_door(door_level_3)  # Передаем узел двери в KillStates
	else:
	...
	if KillStates and kills_label:
		...
		KillStates.set_required_kills(6)  # Устанавливаем, что нужно убить 6 врагов для перехода
	else:
		print("Ошибка: KillStates или KillsLabel не найдены {level 2}")  # Выводим ошибку, если KillStates или KillsLabel не найдены
