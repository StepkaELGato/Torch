...
var words = [
	"Нажмите SPACE, чтобы пропустить катсцену",
	"Ужасная скука…",
...
var textures = [
	preload("res://cutscene/Movement.png"),
...
func update_label_and_image() -> void:
	if labelCount < words.size() - 1:  # Проверяем, не достигли ли мы конца списка слов
		...
		if labelCount == 1 or labelCount == 4 or labelCount == 8:  # Проверяем, нужно ли обновить изображение
